...
class PortfolioPolicy(TimeStampMixin):
    ...
    asset_to_equity_max_ratio = models.DecimalField(
        max_digits=MAX_DIGITS, decimal_places=DECIMAL_PLACES, default=0
    )
    asset_to_equity_min_ratio = models.DecimalField(
        max_digits=MAX_DIGITS, decimal_places=DECIMAL_PLACES, default=0
    )
    debt_to_equity_max_ratio = models.DecimalField(
        max_digits=MAX_DIGITS, decimal_places=DECIMAL_PLACES, default=0
    )
    max_dividend_payout_ratio = models.DecimalField(
        max_digits=MAX_DIGITS, decimal_places=DECIMAL_PLACES, default=0
    )
    minimum_annual_earnings_growth = models.DecimalField(
        max_digits=MAX_DIGITS, decimal_places=DECIMAL_PLACES, default=0
    )
    pe_quantile = models.IntegerField(default=50)
    portfolio = models.OneToOneField(
        Portfolio, on_delete=models.CASCADE, related_name="portfolio_policy"
    )
